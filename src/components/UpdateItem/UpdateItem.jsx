...
const UpdateItem = () => {
    ...
    const getItemsData = async () => {
        const { data } = await axios.get(`/api/v1/getItemById/${idkey.id}`)
        if (data) {
            setName(data.item.name)
            setPlaceCatg(data.category)
            setPrice(data.item.price)
            setdescription(data.item.description)
            setCost(data.item.cost)
            setsku(data.item.SKU)
            setBarcode(data.item.barcode)
            setInstock(data.item.inStock)
            setLowstock(data.item.lowStock)
            setSpicelevel(data.item.spiceLevel)
            setSelectedValue(data.item.selectedValue)
            setAvailable(data.item.available)
            setImgFile(data.item.image)
        }
    }
    useEffect(() => {
        getItemsData()
